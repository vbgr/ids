...
CREATE TABLE ids AS SELECT * FROM read_csv('data/ids');
-- The file does not fave hader so duckdb assigns column name and we need to get it.
SHOW ids;
-- Get total count
SELECT COUNT(*) FROM ids;
-- Get unique count (here "1000000" - is the column name generated by duckdb)
SELECT COUNT(DISTINCT "1000000") FROM ids;
```
